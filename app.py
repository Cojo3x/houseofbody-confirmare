...
PAGINA_DEJA_CONFIRMAT_RAZGANDIT = _SABLON_HTML.format(
    clasa_buton="btn-rosu",
...
def evenimentul_este_confirmat(event):
    """Returneaza True daca titlul evenimentului NU mai contine 'Trebuie',
    adica sedinta a fost deja confirmata printr-o accesare anterioara
    a linkului de confirmare."""
    titlu_curent = event.get("summary", "")
    return "trebuie" not in titlu_curent.lower()


def evenimentul_este_deja_reprogramat(event):
    """Returneaza True daca evenimentul are marcajul 'reprogramat_pentru'
    setat si acesta corespunde cu data curenta de start, adica a fost
    deja mutat printr-o accesare anterioara a linkului de reprogramare."""
    marcaj_data = (
    ...
    return marcaj_data is not None and marcaj_data == data_curenta
...
def confirmare_client(cod, telefon):
    ...
    try:
        ...
        if e.resp.status == 404:
            ...
            # accesare anterioara a linkului de reprogramare (ziua era
            # vineri). Clientul apasa acum, din greseala, vechiul link
            # de confirmare.
            try:
                trimite_email(
                    OWNER_EMAIL,
                    f"Client (tel {telefon}) s-a razgandit.",
                    f"Un client cu numarul de telefon {telefon}, cod programare "
                    f"{cod}, a incercat sa confirme o sedinta care fusese deja "
                    f"reprogramata/anulata.\n\n"
                    f"Contacteaza-l pe WhatsApp pentru clarificari."
                )
            except Exception as e2:
                return f"A aparut o eroare la trimiterea notificarii: {e2}", 500
            return PAGINA_DEJA_CONFIRMAT_RAZGANDIT
        return f"A aparut o eroare la citirea programarii din calendar: {e}", 500
    except Exception as e:
        return f"A aparut o eroare la citirea programarii din calendar: {e}", 500

    # --- Pasul 0.5: verifica daca sedinta a fost DEJA REPROGRAMATA ---
    # (evenimentul exista in continuare, dar a fost mutat printr-o
    # accesare anterioara a linkului de reprogramare, deci acest link
    # de confirmare este vechi / apasat din greseala)
    if evenimentul_este_deja_reprogramat(event):
        nume = extrage_nume_din_titlu(event)
        try:
            trimite_email(
                OWNER_EMAIL,
                f"{nume} s-a razgandit.",
                f"Clientul {nume} (telefon {telefon}), cod programare {cod}, "
                f"a incercat sa confirme o sedinta care fusese deja "
                f"reprogramata.\n\n"
                f"Contacteaza-l pe WhatsApp pentru clarificari."
            )
        except Exception as e:
    ...
    # --- Pasul 1: verifica daca link-ul de confirmare a fost deja activat ---
    if evenimentul_este_confirmat(event):
        return PAGINA_DEJA_CONFIRMAT

    # --- Pasul 2: actualizeaza evenimentul in Google Calendar ---
    try:
        nume = extrage_nume_din_titlu(event)
    ...
    # --- Pasul 3: trimite-ti un EMAIL automat cu link de confirmare finala ---
...
def reprogramare_client(cod, telefon):
    ...
    # --- Pasul 0.5: verifica daca sedinta a fost DEJA CONFIRMATA ---
    # (titlul nu mai contine "Trebuie" => clientul a confirmat deja
    # printr-o accesare anterioara a linkului de confirmare, deci acest
    # link de reprogramare inseamna ca s-a razgandit dupa ce confirmase)
    if evenimentul_este_confirmat(event):
        nume = extrage_nume_din_titlu(event)
        try:
            trimite_email(
                ...
                f"{nume} s-a razgandit.",
                f"Clientul {nume} (telefon {telefon}), cod programare {cod}, "
                f"a confirmat initial sedinta, dar a apasat ulterior si pe "
                f"linkul de reprogramare.\n\n"
                f"Contacteaza-l pe WhatsApp pentru clarificari."
            )
        except Exception as e:
            return f"A aparut o eroare la trimiterea notificarii: {e}", 500
    ...
    # --- Pasul 1: verifica daca link-ul de reprogramare a fost deja folosit ---
    if evenimentul_este_deja_reprogramat(event):
        return PAGINA_DEJA_REPROGRAMAT
